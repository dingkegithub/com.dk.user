...
func (s *Subscribe) sync() {
	...
	s.listener.Range(func(key, f interface{}) bool {
		...
		s.logger.Log("file", "subscribe.go",
			"func", "sync",
			"msg", "service list is update",
			"remote_millis", remoteList.LastRefTime,
			"local_millis", localCacheMillis)

		// 3. whether remote list newer than local cache
		if remoteList.LastRefTime > localCacheMillis.(uint64) {
			...
			s.cacheMile.Store(key, remoteList.LastRefTime)

			// 3.2. notify latest instance list to listener
			f.(SubscribeFunc)(remoteList.LastRefTime, svcId, instMetaList)
		} else {
			s.logger.Log("file", "subscribe.go",
				"func", "sync",
				"msg", "instance list doesn't need update",
				"svcId", svcId,
				"remote_millis", remoteList.LastRefTime,
				"local_millis", localCacheMillis)
		}
