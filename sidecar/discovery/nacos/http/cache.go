...
func (lc LocalCache) HasMemCache(name string) bool  {
...
func (lc *LocalCache) Store(name string, inst *CacheInstance) error {
	...
	if ok {
		if s.Mils >= inst.Mils {
			lc.logger.Log("file", "cache.go",
				"func", "Store",
				"msg", "no need flush local cache",
	...
	if err != nil {
		lc.logger.Log("file", "cache.go",
			"func", "Store",
			"msg", "unmarshal store list failed",
			"size", len(inst.Instances),
			"error", err)
		return nil
	}
	...
	lc.logger.Log("file", "cache.go",
		"func", "update",
		"msg", "write store list into disk",
		"size", size)
